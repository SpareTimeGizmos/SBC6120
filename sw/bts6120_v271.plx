...
;   Copyright (C) 2001-2004 by R. Armstrong, Milpitas, California.
...
; 06000 Front panel CONT, START, BOOT, HALT switches, update display
...
VERSION=271	; latest edit number

; TODO
; Make the DL/RL/DF/RF ommands display the block number in the data LEDs.
; Allow the DF/RF commands to be used with RP (increment the partition
;  number and/or suppress the "Are you sure" prompt)...
...
; checksum.
	POST+3			; set the POST code to three
	CIF	2
...
; Figure out if we have a front panel (FP612) and, if we do, turn it on...
	.PUSHJ	@[FPINIT]
...
; Now print information about the extension ROM, if one was found, but
; it did not initialize properly.  Error 2 means that its checksum was
; invalid.  Error 3 or greater means that the ROM initialization was
; called but it returned this error code.
	TAD	[-2]		; compare the status code to 2
	TAD	EXTFLAG
	SPA
	 JMP	 NOXRMES	; either there is no ROM, or it was OK

	JMS	@ZINLMES	; say
	XRMES1			; "EXT: Error "
	TAD	EXTFLAG		; and display the error code
	.PUSHJ	@ZTDECNW	; ...
	.PUSHJ	@ZCRLF		; followed by a newline
NOXRMES:

;   Now we are ready to initialize the RAM disk array by first testing the
; backup battery and then individually testing each of the eight RAM chips to
...
; discovers...
	JMS	@ZPUSHJ1	; first determine the type of the RAMDisk
	 RDCHK1			; .... and set RDTYPE
	JMS	@ZINLMES	; say
	 RAMMS1			; "NVR: "
	JMS	@ZPUSHJ1	; (cross field call)
	 RDTEST			; test all eight RAM disk units
...
RDBFAI:	JMS	@ZINLMES	; say
	 BFAMSG			; ... "- Battery FAIL"
NORDM:	.PUSHJ	@ZCRLF		; finish the status report and we're done
...
SYSIN9:	JMP	@ZRESTA		; And we're ready for commands...
	NOP			; Hook area for extension ROM
	NOP
...
; Extension ROM info
EXTFLAG: .BLOCK	1	; 0=no ext. ROM, 1=OK, 2=Bad checksum, 3..=error code
...
GETPP:	.PUSHJ	@ZOCTNW		; read the source partition number
	TAD	WORD		; ...
	DCA	CPYSRC		; ...
	.PUSHJ	@[SPATST]	; the next character has to be a space
...
FLLOAD: MM3			; access flash (if it's there)
	CDF	1
...
UPDIS1:	WSR			; display then on the lights
UPDIS2:	CLA CLL			; (just in case!)
	.POPJ			; and we're outta here
...
;;	.POPJ			; ...
	JMP	@[UPDISP]	; keep the front panel display alive and exit
...
; returned in the AC.
INCHRS:	.PUSHJ	@[UPDISP]	; keep the front panel display alive
...
; zero (unknown battery status).
	CDF	0		; the EXTFLAG lives in field 0
	TAD	@[EXTFLAG]	; get the IOB6120 status
	CDF	1		; ...
...
SYSNM3:	.TEXT	/ \d \h/
SYSCRN:	.TEXT	/Copyright (C) 1983-2004 Spare Time Gizmos.  All rights reserved./
...
HLPLST:
	...
	.DATA	HLPPC, HLPPE, HLPFL, HLPPM, HLPB
	; Other (miscellaneous) commands...
	.DATA	HLPNUL, HLPMSC
	.DATA	HLPTW, HLPTP, HLPSC, HLPVE, HLPSEM, HLPRP, HLPDOL
...
HLPPE:	.TEXT	/PE aaaa bbbb\t\t\t-> Compare (Equal) partition aaaa to bbbb/
...
HLPSC:	.TEXT	/SC n\t\t\t\t-> Set console scope on or off/
...
EXTROM: CLA
	CDF	0
	DCA	@[EXTFLAG]	; initialize status to "no extension ROM"
